...
unsigned char* getA(string fileName, int* pSizeOut, bool bAddBasePath, bool bAutoDecompress)
{
	...
	*pSizeOut = ftell(fp);
	...
	/*
	action|log
msg|`4UPDATE REQUIRED!`` : The `$V2.995`` update is now available for your device.  Go get it!  You'll need to install it before you can play online.
			...
			case 2:
			{
				...
				if (cch.find("text|") != std::string::npos){
					...
					else if (str == "/find ")
					{
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Supported commands are: /news, /mods, /online, /help, /mod, /unmod, /inventory, /item id, /team id, /color number, /who, /state number, /count, /sb message, /alt, /radio, /gem, /jsb, /find itemname, /unequip, /weather id, /nick nickname, /unequip"));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
						...
						sendState(peer);
						PlayerDB:(username1, cloth_hair1, cloth_shirt1, cloth_pants1, cloth_feet1, cloth_face1, cloth_hand1, cloth_back1, cloth_mask1, cloth_necklace1, skinColor1);
				...
				if(pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
				{
					...
					GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Server made by Growtopia Noobs and Hosted by LiteG#9999, some fixes by iProgramInCpp and items from Nenkai."));
					ENetPacket * packet = enet_packet_create(p.data,
						p.len,
						ENET_PACKET_FLAG_RELIABLE);
					enet_peer_send(peer, 0, packet);
