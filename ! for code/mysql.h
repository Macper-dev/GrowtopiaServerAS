 Copyright (c) 2000, 2019, Oracle andor its affiliates. All rights reserved.
    ...
    which is part of C Driver for MySQL (ConnectorC), is also subject to the
    Universal FOSS Exception, version 1.0, a copy of which can be found at
    httposs.oracle.comlicensesuniversal-foss-exception.
   ...
   the SHARED_LIB_MAJOR_VERSION in cmakemysql_version.cmake
 ...
 #include mysqlclient_plugin.h   IWYU pragma keep
 ...
 void STDCALL mysql_server_end(void);
   ...
   mysql_server_initend need to be called when using libmysqld or
 ...
 typedef struct MYSQL_BIND {
   ...
   void buffer;           buffer to getput data 
   ...
   void (skip_result)(struct MYSQL_BIND , MYSQL_FIELD , unsigned char row);
    output buffer length, must be set when fetching strbinary 
   unsigned long buffer_length;
   unsigned long offset;               offset position for charbinary fetch 
 ...
 typedef struct MYSQL_STMT {
   ...
   unsigned long prefetch_rows;  number of rows per one COM_FETCH 
     ...
     Copied from mysql-server_status after executefetch to know
